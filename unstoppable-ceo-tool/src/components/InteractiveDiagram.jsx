...
const InteractiveDiagram = memo(({ 
  ...
  // Position for user's typed answer (closer to inner edge)
  const getAnswerPosition = (startAngle, endAngle) => {
    const midAngle = (startAngle + endAngle) / 2;
    const rad = (midAngle - 90) * Math.PI / 180;
    const answerRadius = outerRadius - 45;
    return {
      x: cx + answerRadius * Math.cos(rad),
      y: cy + answerRadius * Math.sin(rad),
      rotation: midAngle
    };
  };
    ...
    <div className="relative w-full flex flex-col items-center">
      <div className="relative w-full flex items-center justify-center gap-1">
        ...
        <svg 
          ...
          className="w-full h-auto flex-1 cursor-pointer" 
          style={{ maxWidth: size === 'large' ? '450px' : '350px' }}
        >
          ...
          {/* Outer segments - clickable */}
          {DIAGRAM_OUTER_SEGMENTS.map((seg, index) => {
            const score = getScore(seg.id);
            const description = getDescription(seg.id);
            ...
            if (hasScore) {
              segmentFill = getScoreColor(score);
            } else if (isActive) {
              segmentFill = '#d4e5ff';
            } else if (isHovered) {
              segmentFill = hoverColor;
            }
            
            const labelPos = getLabelPosition(seg.startAngle, seg.endAngle);
            const answerPos = getAnswerPosition(seg.startAngle, seg.endAngle);
            ...
            return (
              ...
              >
                <motion.path
                  ...
                  fill={segmentFill}
                  stroke={isActive ? '#3b82f6' : strokeColor}
                ...
                {/* User's typed answer - only show if has description */}
                {description && (
                  <text
                    x={answerPos.x}
                    y={answerPos.y}
                    textAnchor="middle"
                    dominantBaseline="middle"
                    transform={`rotate(${textRotation}, ${answerPos.x}, ${answerPos.y})`}
                    fill={strokeColor}
                    fontWeight="500"
                    fontSize="8"
                    opacity="0.7"
...
InteractiveDiagram.displayName = 'InteractiveDiagram';
